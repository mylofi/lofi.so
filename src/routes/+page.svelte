	...
	<div class="mx-auto max-w-[92rem] px-4 sm:px-6 lg:px-8">
		<div class="xl:mr-[19.5rem]">
			<section id="explore" class="space-y-8">
				<header
					class="rounded-2xl border border-slate-200/80 bg-white/90 p-6 shadow-sm backdrop-blur dark:border-gray-800 dark:bg-gray-900/80 sm:p-8"
				>
					...
					<p
						class="mt-3 max-w-3xl text-sm leading-relaxed text-slate-600 dark:text-slate-300 sm:text-base"
					>
						Discover curated tools, connect with the community, and stay updated on the latest in
				...
				<section
					id="join-us"
					class="rounded-2xl border border-slate-200/80 bg-white/90 p-4 shadow-sm backdrop-blur dark:border-gray-800 dark:bg-gray-900/80 sm:p-6"
				>
					<div class="mb-5 flex items-center justify-between gap-4 dark:border-gray-800">
						<div>
							<h3 class="text-xs font-semibold uppercase tracking-[0.16em] text-primary sm:text-sm">
								JOIN
							</h3>
						</div>
					</div>
				...
				<section
					id="learn"
					class="rounded-2xl border border-slate-200/80 bg-white/90 p-6 shadow-sm backdrop-blur dark:border-gray-800 dark:bg-gray-900/80"
				>
					...
					<h4 class="mb-4 text-lg font-semibold text-slate-900 dark:text-slate-100">
						Things to read
					</h4>
					...
					<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
						{#each watchContent.slice(0, 5) as item}
							<article
								class="overflow-hidden rounded-xl border border-slate-200 bg-white transition hover:border-primary/20 hover:shadow-sm dark:border-gray-700 dark:bg-gray-900"
							>
				...
				>
					<div class="mb-8 flex items-center justify-between gap-4 dark:border-gray-800">
						...
						<div class="flex flex-wrap items-center justify-end gap-2">
							...
							>
								<span
									class="inline-flex h-8 w-8 items-center justify-center rounded-md bg-primary text-sm font-black tracking-[0.08em] text-white"
								>
									LF
								</span>
								<span>LF News</span>
					...
					<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
						{#each mentions.mentions.slice(0, 3) as mention}
							<article
								class="rounded-xl border border-slate-200 bg-white p-4 transition hover:border-primary/20 hover:shadow-sm dark:border-gray-700 dark:bg-gray-900"
							>
								...
								{#each tab.items.slice(0, 6) as item}
									...
									>
										<div class="flex items-start gap-3">
											<img src={item.icon} alt={item.title} class="h-12 w-12 rounded-xl" />
											<div class="flex-1">
										...
										<span
											class="mt-3 inline-flex rounded-full border border-primary/20 bg-primary/10 px-2.5 py-1 text-xs font-semibold text-primary dark:border-primary/30 dark:bg-primary/20"
										>
											{tab.label}
										</span>
									</a>
