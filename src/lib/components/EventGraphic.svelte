	...
	<main id="graphic" class="flex items-center justify-center p-4">
		...
		>
			...
			<section class="relative flex w-full flex-col gap-3 p-2 sm:gap-4 sm:p-3 lg:w-auto">
				<h3 class="m-0 text-center text-lg font-semibold text-white sm:text-xl">
					{#if isEventPassed}
						Catch Up!
					{:else}
						Join Us!
				...
				<div class="hidden flex-col items-center gap-2 lg:flex">
					<div class="h-24 w-24 rounded-lg bg-white p-2 sm:h-28 sm:w-28 lg:h-32 lg:w-32">
						<img
							src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${isEventPassed ? 'https://www.youtube.com/playlist?list=PLTbD2QA-VMnXFsLbuPGz1H-Najv9MD2-H' : eventData.registrationUrl}`}
							alt="Event QR Code"
							class="h-full w-full"
						/>
					</div>
					<span class="text-sm text-white/90">Scan for {isEventPassed ? 'recordings' : 'details'}</span>
				</div>
				<!-- Action Buttons -->
